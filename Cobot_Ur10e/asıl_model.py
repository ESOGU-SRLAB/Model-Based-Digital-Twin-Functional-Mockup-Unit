...
#    Bu örnek, UR10e’nin tipik DH parametrelerinden yola çıkarak basitleştirilmiştir.
...
###############################################################################
# Newton-Euler Dinamik Hesaplamalar (Basit Model)
###############################################################################
def compute_dynamics(q, dq, ddq):
    """
    Basit Newton-Euler yaklaşımı:
    tau = I*ddq + gravity_term
    Burada sadece diagonal inertia ve basit gravity kullanılmıştır.
    """
    tau = np.zeros(NUM_JOINTS)
    I_approx = np.array([0.1, 0.1, 0.05, 0.02, 0.02, 0.01])
    for i in range(NUM_JOINTS):
        inertia_term = I_approx[i] * ddq[i]
        grav_term = LINK_MASSES[i] * GRAVITY * 0.1 * math.sin(q[i])
        tau[i] = inertia_term + grav_term
    return tau

###############################################################################
# FMU Class for UR10e Cobot
###############################################################################
class Model(Fmi2FMU):
    """
    FMI Co-Simulation model for UR10e robot:
      - Ters kinematik ile end-effector hedef pose (x_des, y_des, z_des, roll_des, pitch_des, yaw_des)
    ...
    def do_step(self, currentTime, stepSize, noSetFMUStatePriorToCurrentPoint):
        ...
        # 3) Dinamik hesaplama (gravity + inertia approx)
