...
def apply_normal_inputs(fmu):
    ...
    According to modelDescription.xml, the inputs are:
    ...
    return normal_inputs  # Return for later use
...
# Create the FMU instance (this is the FMU model defined in model.py)
...
# Prepare arrays to log simulation data
...
# Apply normal inputs once and compute the desired joint angles (reference)
...
for i in range(num_steps):
    t = i * dt
    # Reapply the constant normal inputs at each time step
    apply_normal_inputs(fmu)
    
    # Execute one simulation step
    fmu.do_step(t, dt, True)
    
    # Retrieve outputs from the FMU:
    # - Joint angles: valueRefs 8-13
    # - Joint velocities: valueRefs 14-19
    # - Joint torques: valueRefs 26-31
    ...
    # Perform an external safety check
...
for j in range(NUM_JOINTS):
    ...
    # Annotate 10% and 90% thresholds based on desired target
